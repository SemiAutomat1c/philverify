...
class TestVerifyText:
    def test_valid_text_returns_200(self):
        res = client.post("/api/verify/text", json={
    ...
    def test_too_short_text_returns_422(self):
        res = client.post("/api/verify/text", json={"text": "Short"})
    ...
    def test_empty_body_returns_422(self):
        res = client.post("/api/verify/text")
        assert res.status_code == 422
...
class TestVerifyUrl:
    def test_invalid_url_returns_422(self):
        res = client.post("/api/verify/url", json={"url": "not-a-url"})
        assert res.status_code == 422

    def test_missing_url_returns_422(self):
        res = client.post("/api/verify/url", json={})
        assert res.status_code == 422
    ...
    def test_valid_url_format_accepted(self):
        ...
        res = client.post("/api/verify/url", json={"url": "https://rappler.com/fake-article-test"})
...
class TestTrends:
    ...
    def test_trends_days_out_of_range(self):
        res = client.get("/api/trends?days=0")
        assert res.status_code == 422
