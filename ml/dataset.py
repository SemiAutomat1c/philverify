...
DATASET: list[Sample] = [
    ...
    Sample("May kumakalat na screenshot na suspended daw ang lahat ng klase bukas pero wala pang LGU post", 1),
    Sample("Unconfirmed: may ayuda raw na P10,000 para sa bawat pamilya sa NCR ngayong linggo", 1),
    Sample("May nagpost na magtataas daw ang pamasahe next week pero walang LTFRB announcement", 1),
    Sample("Ayon daw sa insider, may bagong curfew sa buong Metro Manila starting tonight", 1),
    Sample("Hindi pa verified ang claim na may bagong travel ban para sa OFWs papuntang Middle East", 1),
    Sample("May viral message na magsasara raw ang ilang bangko sa Biyernes pero walang BSP notice", 1),
    Sample("Unverified report says a mayor was arrested last night but no police statement is linked", 1),
    Sample("May claim na required na raw ang bagong vaccine card sa lahat ng opisina next month", 1),
    Sample("Alleged leaked memo says schools will shift back to full online classes this semester", 1),
    Sample("May post na cancelled daw lahat ng domestic flights dahil sa bagyo pero walang airline advisory", 1),
    Sample("Source says may malaking oil price rollback next week pero wala pang official announcement", 1),
    Sample("May kumakalat na listahan ng barangay cash aid beneficiaries pero hindi verified ang source", 1),
    Sample("Unconfirmed claim: a popular mall chain will close all branches nationwide this year", 1),
    Sample("May rumor na magpapalit daw ng school calendar ang DepEd pero walang official memo", 1),
    Sample("Claim circulating online says checkpoints will be set up on all expressways tonight", 1),
    Sample("May nagsasabing may bagong SIM registration round next week pero wala pang DICT source", 1),
    Sample("Unverified screenshot claims a lawmaker voted against a bill they publicly supported", 1),
    Sample("May post na may food shortage daw sa buong Luzon pero walang cited news report", 1),
    Sample("Allegedly, a celebrity endorsed a political candidate in a private event, according to fans", 1),
    Sample("Hindi pa confirmed ang post na magkakaroon ng free electricity subsidy for all households", 1),
